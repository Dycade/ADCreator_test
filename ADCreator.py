...
def generate_AD(driver,domain,base_sid,states,country,level,num_users,num_pcs):
    with driver.session() as session:
        ...
        if level == 'Medium':
            ...
            for user in random.sample(T0_users,int(len(T0_users)*0.1)):
                num_sessions = random.randrange(0, max_sessions_per_user)
                if num_sessions == 0:
                    continue
                for c in random.sample(Workingstation, num_sessions):
        ...
        session.run(
                'MATCH (n:Computer) WHERE (n.name CONTAINS $domain_name) MATCH (m:Group {name:$gname}) MERGE (m)-[:AdminTo]->(n)',domain_name = domain, gname=cn("DOMAIN ADMINS",domain))
        for pc in T1_PAW:
            session.run(
                'MATCH (n:Group {name:$gname}) WHERE (n.name CONTAINS $domain_name) MERGE (m:Computer {name:$pname}) MERGE (n)-[:AdminTo]->(m)',domain_name = domain, gname=cn("T1 Admins",domain),pname = pc)
        for pc in T2_PAW:
            session.run(
                'MATCH (n:Group {name:$gname}) WHERE (n.name CONTAINS $domain_name) MERGE (m:Computer {name:$pname}) MERGE (n)-[:AdminTo]->(m)',domain_name = domain, gname=cn("T2 Admins",domain),pname = pc)
    ...
    return Workingstation,T2_users


if __name__ == '__main__':
    do_cleardb()
    generate_AD(driver,domain,base_sid,states,country,level,num_users,num_pcs)
