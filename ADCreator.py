    ...
    for i in depart:
        ...
        for j in states:
            ...
            session.run(
                'UNWIND $props as prop MERGE (n:Base {objectid:prop.id}) SET n:Group, n.name=prop.name', props=props)
            session.run(
                'MERGE (n:Group {name:$name}) WITH n MERGE (m:OU {name:$gname}) WITH n,m MERGE (m)-[:Contains]->(n)', name=group, gname=cn('Distribution Groups',domain))
            session.run(
        ...
        for l in ACL:
            for n in range(random.randint(0, 5)):
                ...
                session.run(
                    'UNWIND $props as prop MERGE (n:Base {objectid:prop.id}) SET n:Group, n.name=prop.name', props=props)
                session.run(
                    'MERGE (n:Group {name:$name}) WITH n MERGE (m:OU {name:$gname}) WITH n,m MERGE (m)-[:Contains]->(n)', name=group, gname=cn('Security Groups',domain))
                session.run(
