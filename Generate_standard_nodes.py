...
def Country_states(Country):
    if Country == "US":
        return ["NY", "CA", "DE", "FL", "WA"]
    if Country == "CANADA":
        return ["BC", "ON", "AB", "QC"]
    if Country == "AUSTRALIA":
        return ["NSW", "QLD", "SA", "WA", "VIC"]
...
def generate_standard_nodes(driver,domain,base_sid,type):
    with driver.session() as session:
        ...
        if type == 'root':
            ...
            country = random.choice(['US','CANADA','AUSTRALIA'])
            state = random.choice(Country_states(country))
            group_name = cn("DOMAIN CONTROLLERS",domain)
            comp_name = cn(f"DC-root-"+str(state),domain)
            sid = cs(1000,domain)
            session.run(
