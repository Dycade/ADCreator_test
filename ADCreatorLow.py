...
def generate_low(driver,domain,base_sid,states,country,level,num_users,num_pcs):
    with driver.session() as session:
        ...
        session.run(
            'MATCH (n:Computer) WHERE (n.name CONTAINS $domain_name) MATCH (m:Group {objectid: $id}) MERGE (m)-[:AdminTo]->(n)',domain_name = domain, id=cs(512,base_sid))
    ...
    return computers,list(set(users) - set(das))

if __name__ == '__main__':
    do_cleardb()
    generate_low(driver,domain,base_sid,states,country,level,num_users,num_pcs)